...
const BRANCHES = [
  ...
  "Electronics & Communication","Computer Engineering","Chemical Engineering",
  "IT Engineering","Automobile Engineering","Other",
];
const SECTIONS: Array<"Science" | "Math" | "English"> = ["Science", "Math", "English"];

interface TestConfig { science: number; math: number; english: number; }
const DEFAULT_CONFIG: TestConfig = { science: 50, math: 30, english: 20 };

const loadConfig = (): TestConfig => {
  try { const r = LS.get("ddcet:config"); if (r) return JSON.parse(r) as TestConfig; } catch {}
  return DEFAULT_CONFIG;
};
const saveConfig = (c: TestConfig): void => { LS.set("ddcet:config", JSON.stringify(c)); };
...
const uid = (): string => `${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
const loadQ = (): Question[] => {
  try {
    ...
    if (r) { const q = JSON.parse(r) as Question[]; if (Array.isArray(q) && q.length > 0) return q; }
  } catch {}
...
// ═══════════════════════════════════════════════
// DEFAULT QUESTIONS (100 total)
// ═══════════════════════════════════════════════
const DEFAULT_QUESTIONS: Question[] = [
  // ── SCIENCE (50) ──────────────────────────────
  { id:1,  section:"Science", text:"CaCO₃(s) → CaO(s) + CO₂(g). Which kind of reaction is this?", options:["Displacement reaction","Oxidation reaction","Thermal decomposition reaction","Precipitation reaction"], answer:2 },
  { id:2,  section:"Science", text:"What is the formula of lead sulphate?", options:["Pb₂SO₄","Pb(SO₄)₂","PbSO₄","Pb₂(SO₄)₃"], answer:2 },
  { id:3,  section:"Science", text:"Balanced equation of Al + HCl → AlCl₃ + H₂?", options:["Al + 2HCl → AlCl₂ + H₂","2Al + 6HCl → 2AlCl₃ + 3H₂","3Al + 2HCl → 2AlCl₂ + 3H₂","4Al + 3HCl → AlCl₃ + 3H₂"], answer:1 },
  { id:4,  section:"Science", text:"Main product of NaOH reacting with zinc metal?", options:["NaZnO₂","NaZn₂O","Na₂ZnO₂","Zn(OH)₂"], answer:2 },
  { id:5,  section:"Science", text:"Which type of oxide is called basic oxide?", options:["Non-metallic oxide","Metallic oxide","Both A and B","Inactive oxide"], answer:1 },
  { id:6,  section:"Science", text:"Which compound is known as milk of magnesia?", options:["Mn(OH)₂","Mg(OH)₂","MgO","Mg(OH)"], answer:1 },
  { id:7,  section:"Science", text:"Chemical formula of bleaching powder:", options:["Ca(OH)₂","CaOCl₂","Ca₂OCl","CaCO₃·2H₂O"], answer:1 },
  { id:8,  section:"Science", text:"Which is the lustrous non-metal?", options:["Iodine","Carbon","Nitrogen","Bromine"], answer:0 },
  { id:9,  section:"Science", text:"Which metal reacts with steam but not cold/hot water?", options:["Aluminium","Iron","Zinc","All of the above"], answer:3 },
  { id:10, section:"Science", text:"Correct order of reactivity of metals:", options:["Mg > Al > Zn > Fe","Mg > Fe > Zn > Al","Fe > Zn > Al > Mg","Mg > Al > Fe > Zn"], answer:0 },
  { id:11, section:"Science", text:"The smallest unit of data in a computer:", options:["Byte","Bit","Nibble","GB"], answer:1 },
  { id:12, section:"Science", text:"Ctrl+Home in MS-Word moves cursor to:", options:["End of document","Beginning of line","Beginning of paragraph","Beginning of document"], answer:3 },
  { id:13, section:"Science", text:"Which is a relative link in HTML?", options:['href="http://www.blog.com"','href="../index.html"','href="http://index.html"',"All of the above"], answer:1 },
  { id:14, section:"Science", text:"Correct MS-Excel formula for total of A2,B2,C2,D2:", options:["=SUM(A2:D2)","=SUM(A2,B2,C2,D2)","Both A and B","None of the above"], answer:2 },
  { id:15, section:"Science", text:"Which is NOT valid in MS-PowerPoint View menu?", options:["Tree view","Outline view","Notes page","Reading view"], answer:0 },
  { id:16, section:"Science", text:"Green plants that make their own food are:", options:["Heterotrophs","Autotrophs","Saprotrophs","Omnivores"], answer:1 },
  { id:17, section:"Science", text:"Bag filters are used to:", options:["Reduce noise pollution","Reduce sludge from waste water","Remove particulate matter from gas stream","Reduce soil pollution"], answer:2 },
  { id:18, section:"Science", text:"Energy from heat trapped inside earth is:", options:["Geo-Thermal energy","Hydrothermal energy","Solar energy","Wave energy"], answer:0 },
  { id:19, section:"Science", text:"Device converting solar energy to electricity:", options:["Fuel cell","Hydrogen cell","Solar array","Photovoltaic cell"], answer:3 },
  { id:20, section:"Science", text:"Paris Agreement temperature rise limit above pre-industrial levels:", options:["1.0°C","1.5°C","2.0°C","2.5°C"], answer:2 },
  { id:21, section:"Science", text:"Significant digits in 2025:", options:["3","4","2","5"], answer:1 },
  { id:22, section:"Science", text:"Zero error is an example of _____ error.", options:["Systematic","Random","Maximum","Minimum"], answer:0 },
  { id:23, section:"Science", text:"1 joule = _____ erg.", options:["10³","10⁵","10⁷","10⁹"], answer:2 },
  { id:24, section:"Science", text:"Density of water in CGS system:", options:["1000","100","10","1"], answer:3 },
  { id:25, section:"Science", text:"% error: mass=0.3%, radius=0.5%. % error in density of sphere:", options:["1.8%","1.5%","1.2%","1.1%"], answer:0 },
  { id:26, section:"Science", text:"Pitch=1mm, 100 divisions. Least count of micrometer:", options:["1 mm","0.1 mm","0.01 mm","0.001 mm"], answer:3 },
  { id:27, section:"Science", text:"SI Unit of linear momentum:", options:["Kg s/m","Kg m/s","m s/Kg","Kg s m"], answer:1 },
  { id:28, section:"Science", text:"Centripetal force always acts:", options:["Away from centre","In tangential direction","Perpendicular to motion","Towards centre"], answer:3 },
  { id:29, section:"Science", text:"External force=0, acceleration:", options:["Increases","Decreases","Equals zero","Remains constant"], answer:2 },
  { id:30, section:"Science", text:"Angular velocity formula (speed v, radius r):", options:["v/r","vr","vr²","vr³"], answer:0 },
  { id:31, section:"Science", text:"Mass=10 kg, acceleration=9.8 m/s². Force:", options:["9.8 N","98 N","0.98 N","980 N"], answer:1 },
  { id:32, section:"Science", text:"50 kg, radius=2 m, speed=60 m/s. Centripetal acceleration:", options:["6000 m/s²","3000 m/s²","1800 m/s²","1500 m/s²"], answer:2 },
  { id:33, section:"Science", text:"SI Unit of electric flux:", options:["C²/Nm","Nm/C","Nm/C²","Nm²/C"], answer:3 },
  { id:34, section:"Science", text:"Electric field lines are:", options:["Imaginary","Real","Ideal","None of the above"], answer:0 },
  { id:35, section:"Science", text:"Electrons in 1 coulomb charge:", options:["6.25×10¹⁷","6.25×10¹⁸","6.25×10¹⁹","6.25×10²⁰"], answer:1 },
  { id:36, section:"Science", text:"Dielectric between capacitor plates, capacitance:", options:["Increases","Decreases","Remains constant","Becomes zero"], answer:0 },
  { id:37, section:"Science", text:"Distance between charges doubled, electric force:", options:["Half","Double","One fourth","Four times"], answer:2 },
  { id:38, section:"Science", text:"Effective resistance of 5Ω, 10Ω, 15Ω in series:", options:["10 Ω","30 Ω","20 Ω","5.5 Ω"], answer:1 },
  { id:39, section:"Science", text:"Heat capacity per unit mass:", options:["Specific heat","Relative heat","Cp","Cv"], answer:0 },
  { id:40, section:"Science", text:"1 KCal = _____ J.", options:["4.186","4186","41.86","418.6"], answer:1 },
  { id:41, section:"Science", text:"Glass heated then cooled — crack forms. Reason:", options:["High melting point","Large thermal conductivity","Large specific heat","Small thermal conductivity"], answer:3 },
  { id:42, section:"Science", text:"Heat given to boiling water, temperature:", options:["Increases","Decreases","Remains constant","None of above"], answer:2 },
  { id:43, section:"Science", text:"37°C = _____ °F.", options:["98.6","98.8","97.6","96.8"], answer:0 },
  { id:44, section:"Science", text:"SI Unit of coefficient of thermal conductivity:", options:["J/m K","N/m K","W/m K","m K/W"], answer:2 },
  { id:45, section:"Science", text:"Audible wave frequency range:", options:["20 Hz–10 KHz","20 Hz–20 KHz","10 Hz–10 KHz","10 Hz–20 KHz"], answer:1 },
  { id:46, section:"Science", text:"Speed of light in vacuum:", options:["3×10⁵ m/s","3×10⁶ m/s","3×10⁷ m/s","3×10⁸ m/s"], answer:3 },
  { id:47, section:"Science", text:"Periodic time of wave with frequency 20 Hz:", options:["0.05 s","0.5 s","5 s","50 s"], answer:0 },
  { id:48, section:"Science", text:"LASER is extremely _____ radiation.", options:["Unidirectional","Focused","Coherent and Stimulated","All of above"], answer:3 },
  { id:49, section:"Science", text:"Auditorium: vol=5000 m³, absorption=900 OWU. Reverberation time:", options:["8.094 s","0.729 s","0.894 s","1.003 s"], answer:2 },
  { id:50, section:"Science", text:"Numerical aperture of optical fiber (n₁=core, n₂=cladding):", options:["√(n₁²−n₂²)","√(n₂²−n₁²)","√(n₁−n₂)","n₁−n₂"], answer:0 },
  // ── MATH (30) ──────────────────────────────────
  { id:51, section:"Math", text:"d/dx [log(sin x)] =", options:["cot x","–cot x","tan x","–tan x"], answer:0 },
  { id:52, section:"Math", text:"If x + y = –2 then dy/dx =", options:["0","1","2","–1"], answer:3 },
  { id:53, section:"Math", text:"∫(1/eˣ)dx = _____ + C", options:["e⁻ˣ","–e⁻ˣ","eˣ","–eˣ"], answer:1 },
  { id:54, section:"Math", text:"log125 / log5 =", options:["log125–log5","log120","25","3"], answer:3 },
  { id:55, section:"Math", text:"log2 + 2log3 =", options:["2log6","log8","log18","log12"], answer:2 },
  { id:56, section:"Math", text:"Mean of first five negative integers:", options:["15","–15","3","–3"], answer:3 },
  { id:57, section:"Math", text:"225° = _____ radian.", options:["3π/4","5π/4","7π/4","9π/4"], answer:1 },
  { id:58, section:"Math", text:"If sinθ = 2/5, then cotθ =", options:["1","1/2","2","1/5"], answer:0 },
  { id:59, section:"Math", text:"Principal period of cos(3+4x) =", options:["π/4","–π/4","π/2","–π/2"], answer:2 },
  { id:60, section:"Math", text:"|î + ĵ + k̂| =", options:["3","–3","√3","9"], answer:2 },
  { id:61, section:"Math", text:"If lines x–2y=3 and 3y+px–1=0 are perpendicular, p =", options:["3/2","–3/2","6","–6"], answer:2 },
  { id:62, section:"Math", text:"Centre of circle (x+3)² + y² – 2 = 0 is:", options:["(3,0)","(0,3)","(–3,0)","(0,–3)"], answer:2 },
  { id:63, section:"Math", text:"If f(x) = eˡᵒᵍˣ, f(1) =", options:["0","1","e","–e"], answer:1 },
  { id:64, section:"Math", text:"lim(x→0) sin2x / tanx =", options:["1","1/2","2","3"], answer:2 },
  { id:65, section:"Math", text:"d/dx[log(5sinθ+3)] =", options:["0","cosθ/(5sinθ+3)","–cosθ/(5sinθ+3)","cosθ/3"], answer:0 },
  { id:66, section:"Math", text:"d/dx(cos²x) =", options:["sin2x","–sin2x","cos2x","–cos2x"], answer:1 },
  { id:67, section:"Math", text:"∫cos(10x+17)dx = _____ + C", options:["10sin(10x+17)","–10sin(10x+17)","(1/10)sin(10x+17)","–(1/10)sin(10x+17)"], answer:2 },
  { id:68, section:"Math", text:"f(x) = log₄x. Find f(64).", options:["3","–3","6","1/3"], answer:0 },
  { id:69, section:"Math", text:"If vectors (2,3,a) and (3,–1,4) are perpendicular, a =", options:["–3/4","3/4","4/3","–4/3"], answer:1 },
  { id:70, section:"Math", text:"If centre of x²+y²–6my+11=0 is (0,–3), m =", options:["2","–2","1","–1"], answer:0 },
  { id:71, section:"Math", text:"Equation of line through (1,–1) and (–1,1):", options:["2x+y–1=0","x+2y–1=0","x+y=0","x+2y+1=0"], answer:2 },
  { id:72, section:"Math", text:"∫3x/(x²+6x+40)dx = _____ + C", options:["(1/2)log|x²+6x+40|","–(1/2)log|x²+6x+40|","2log|x²+6x+40|","–2log|x²+6x+40|"], answer:0 },
  { id:73, section:"Math", text:"log₁/₉(81) =", options:["–2","2","–1/2","1/2"], answer:0 },
  { id:74, section:"Math", text:"log₂3 + log₈27 + 4·2 =", options:["9","10","11","12"], answer:2 },
  { id:75, section:"Math", text:"Mean of 19,15,12,k,8,3 is 11. Find k.", options:["8","9","10","11"], answer:1 },
  { id:76, section:"Math", text:"d/dx(sin5x·cos3x) =", options:["5cos5x·cos3x – 3sin5x·sin3x","5cos5x·cos3x + 3sin5x·sin3x","–5cos5x·cos3x – 3sin5x·sin3x","cos5x·cos3x"], answer:0 },
  { id:77, section:"Math", text:"|A| where A=[1 2 –1; 2 1 3; –1 1 0]", options:["–3","3","–4","4"], answer:2 },
  { id:78, section:"Math", text:"If A=[2 –1; 3 1], then A⁻¹ =", options:["(1/5)[1 1; –3 2]","(1/5)[2 1; –3 1]","(1/5)[–1 1; 3 2]","(1/5)[2 3; 1 1]"], answer:0 },
  { id:79, section:"Math", text:"sin(225°) =", options:["–1/√2","1/√2","√3/2","1/2"], answer:0 },
  { id:80, section:"Math", text:"If A=[2 –1;1 0], B=[1 0;2 3], then 3B–2A =", options:["[–1 2; 4 9]","[–1 –2; 4 9]","[1 2; 4 9]","[1 –2; 4 9]"], answer:2 },
  // ── ENGLISH (20) ──────────────────────────────
  { id:81,  section:"English", text:"'The talented singer danced quickly.' Identify the adverb.", options:["talented","quickly","danced","singer"], answer:1 },
  { id:82,  section:"English", text:"'The captain along with ten players _____ arrived.'", options:["has","have","is","are"], answer:0 },
  { id:83,  section:"English", text:"'None of the products _____ up to the mark.'", options:["is","are","was","were"], answer:3 },
  { id:84,  section:"English", text:"'There is a church _____ the lake.'", options:["over","on","between","above"], answer:3 },
  { id:85,  section:"English", text:"'He _____ to the gym every morning before work.'", options:["goes","went","is going","go"], answer:0 },
  { id:86,  section:"English", text:"Select the correct spelling:", options:["Fahrenhit","Ferrenhit","Fahrenheit","Fahrenheat"], answer:2 },
  { id:87,  section:"English", text:"Select the correct spelling:", options:["Inconvenience","Inconveninse","Inconvinience","Inconviniance"], answer:0 },
  { id:88,  section:"English", text:"Understanding the original idea of a message is called:", options:["Decoding","Feedback","Encoding","Transmission"], answer:0 },
  { id:89,  section:"English", text:"Study of body language including facial expressions:", options:["Proxemics","Chronemics","Paralanguage","Kinesics"], answer:3 },
  { id:90,  section:"English", text:"Which is NOT an example of verbal communication?", options:["Manager giving oral instructions","Man shaking hands","Student writing a letter","Old man reading newspaper"], answer:1 },
  { id:91,  section:"English", text:"A letter of adjustment is written in response to:", options:["Letter of inquiry","Request letter","Order letter","Complaint letter"], answer:3 },
  { id:92,  section:"English", text:"A letter of inquiry is written to:", options:["Offer a product","Seek details about product/service","Start an investigation","Raise a complaint"], answer:1 },
  { id:93,  section:"English", text:"Which is an essential part of a business letter?", options:["Executive summary","Identification initials","Inside address","Carbon copy notation"], answer:2 },
  { id:94,  section:"English", text:"Full form of 'PS' in letter writing:", options:["Please","Post Service","Personnel Service","Post Script"], answer:3 },
  { id:95,  section:"English", text:"'Hurrah! I have passed the examination.' Identify the interjection.", options:["Examination","Passed","Hurrah!","The whole sentence"], answer:2 },
  { id:96,  section:"English", text:"'The dog is sitting _____ the cot.'", options:["under","into","between","up"], answer:0 },
  { id:97,  section:"English", text:"'The Sun _____ in the East.'", options:["raise","rises","rise","raised"], answer:1 },
  { id:98,  section:"English", text:"'Your family _____ a happy family.'", options:["are","is","has","have"], answer:1 },
  { id:99,  section:"English", text:"Choose the correct grammatical sentence:", options:["People were coming, going and ignorant him.","People were coming, going and ignoring him.","People where coming, going and ignoring him.","People were coming, going and ignored him."], answer:1 },
  { id:100, section:"English", text:"Verbal communication can be in _____ form.", options:["written","oral and written","oral","none of these"], answer:1 },
];
...
export default function App() {
  ...
  return (
    <>
      ...
      {screen === "admin"       && <Admin       setScreen={setScreen} questions={questions} setQuestions={setQuestions} config={config} setConfig={setConfig} />}
    </>
  );
}
    ...
    <div className="pg home-pg">
      <div className="home-grid">
        <div className="home-left">
          ...
          <h1 className="home-h1">Mock Test<br />Platform</h1>
          <p className="home-sub">Diploma to Degree Common Entrance Test — Official Pattern Practice</p>
          <div className="info-pills">
            <div className="ipill"><b>{config.science + config.math + config.english}</b>Questions</div>
            <div className="ipill"><b>150</b>Minutes</div>
            <div className="ipill"><b>{(config.science + config.math + config.english) * 2}</b>Marks</div>
          </div>
          <div className="dist-grid">
            {([["Science", config.science, "#3b82f6"], ["Math", config.math, "#8b5cf6"], ["English", config.english, "#10b981"]] as [string, number, string][]).map(([s, n, c]) => (
              <div key={s} className="dist-row">
                <div className="dist-bar" style={{ width: `${(n / Math.max(config.science + config.math + config.english, 1)) * 100}%`, background: c }} />
                <span className={`sec-tag sec-${s}`}>{s}</span>
                <span style={{ marginLeft: "auto", fontSize: ".75rem", color: "#64748b" }}>{n} Q</span>
              </div>
            ))}
          </div>
        ...
        <div className="home-right">
          <div className="form-card">
            ...
            <div className="fg">
              ...
              <input className={`fi${nameErr ? " fi-e" : ""}`} placeholder="Enter your full name"
                value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === "Enter" && start()} />
              {nameErr && <span className="fe">{nameErr}</span>}
            </div>

            <div className="fg">
              <label className="fl">Engineering Branch *</label>
              <select className={`fi${branchErr ? " fi-e" : ""}`} value={branch} onChange={e => setBranch(e.target.value)}>
                <option value="">— Select branch —</option>
                {BRANCHES.map(b => <option key={b}>{b}</option>)}
              </select>
              {branchErr && <span className="fe">{branchErr}</span>}
            </div>
            ...
            <button className="btn-prim" onClick={start}>Start Test →</button>
            <div className="divider"><span>or</span></div>
            <button className="btn-out w100" onClick={() => setScreen("leaderboard")}>🏆 View Leaderboard</button>
            <button className="btn-ghost" onClick={() => setShowAdm(v => !v)}>⚙ Admin Panel</button>

            {showAdm && (
              <div style={{ marginTop: ".6rem", display: "flex", flexDirection: "column", gap: 6 }}>
                <input className="fi" type="password" placeholder="Admin password" value={pw}
                  onChange={e => { setPw(e.target.value); setPwErr(""); }}
                  onKeyDown={e => e.key === "Enter" && enterAdmin()} />
                {pwErr && <span className="fe">{pwErr}</span>}
                <button className="btn-adm" onClick={enterAdmin}>Enter Admin →</button>
              </div>
            )}
          </div>
        </div>
...
function Test({ setScreen, questions, config }: { setScreen: (s: Screen) => void; questions: Question[]; config: TestConfig }) {
  ...
  const [testQs] = useState<Question[]>(() => {
    const pick = (arr: Question[], n: number) => shuffle(arr).slice(0, Math.min(n, arr.length));
  ...
  const chipCls = (i: number): string => {
    const a = ans[i] !== undefined;
    const f = flag[i];
    return a && f ? "chip-both" : f ? "chip-flag" : a ? "chip-ans" : i === cur ? "chip-cur" : "chip-none";
  };
    ...
    <div className="test-root">
      ...
      <div className="t-hdr">
        <div className="thdr-l">
          ...
          <span className="t-branch">{student.branch.split(" ").slice(0, 2).join(" ")}</span>
        </div>
        ...
        <div className="thdr-r">
          ...
          <button className="btn-tog" onClick={() => setPanel(v => !v)}>☰</button>
          <button className="btn-sub" onClick={() => {
            if (window.confirm(`Submit?\n${testQs.length - attempted} unanswered question(s).`)) submit();
          }}>Submit</button>
        </div>
      </div>
      ...
      {/* Progress bar */}
      <div className="prog">
        <div className="prog-f" style={{ width: `${((cur + 1) / testQs.length) * 100}%` }} />
      </div>
      ...
      <div className="t-body">
        ...
        <div className="q-main">
          <div className="q-meta-row">
            ...
            <span className="q-num">Q {cur + 1} / {testQs.length}</span>
          </div>
          <div className="q-txt">{q.text}</div>
          <div className="opts-col">
            {q.options.map((o, i) => (
              <button key={i} className={`opt${ans[cur] === i ? " opt-sel" : ""}`}
          ...
          <div className="q-foot">
            <button className={`btn-flag${flag[cur] ? " fl-on" : " fl-off"}`}
              onClick={() => setFlag(f => ({ ...f, [cur]: !f[cur] }))}>
              {flag[cur] ? "🚩 Flagged" : "🏳 Flag"}
            </button>
            <div className="nav-r">
              {ans[cur] !== undefined && (
                <button className="btn-clr" onClick={() => setAns(a => { const n = { ...a }; delete n[cur]; return n; })}>
                  ✕ Clear
                </button>
              )}
              <button className="btn-nav" disabled={cur === 0} onClick={() => setCur(c => c - 1)}>← Prev</button>
              <button className="btn-nav prim" disabled={cur === testQs.length - 1} onClick={() => setCur(c => c + 1)}>Next →</button>
            </div>
          </div>
        </div>
        ...
        <div className={`nav-panel${panel ? " pan-open" : ""}`}>
          <div className="np-hdr">
            ...
            <button className="np-cls" onClick={() => setPanel(false)}>✕</button>
          </div>
          <div className="np-flt">
            {(["All", "Science", "Math", "English"] as SectionFilter[]).map(s => (
              <button key={s} className={`nfb${sf === s ? " nfa" : ""}`} onClick={() => setSf(s)}>{s}</button>
            ))}
          </div>
          ...
          <div className="np-grid">
            {navQs.map(({ i }) => (
              <div key={i} className={`chip ${chipCls(i)}${i === cur ? " chip-cur" : ""}`}
                onClick={() => { setCur(i); if (window.innerWidth < 900) setPanel(false); }}>
                {i + 1}
              </div>
            ))}
          </div>
...
function Result({ setScreen }: { setScreen: (s: Screen) => void }) {
  ...
  const gc    = pct >= 75 ? "#4ade80" : pct >= 60 ? "#60a5fa" : pct >= 45 ? "#fbbf24" : pct >= 30 ? "#f97316" : "#f87171";
        ...
        <div className="lb-flt">
          {branches.slice(0, 8).map(b => (
            <button key={b} className={`nfb${flt === b ? " nfa" : ""}`}
              style={{ fontSize: ".71rem" }} onClick={() => setFlt(b)}>
              {b === "All" ? b : b.split(" ")[0]}
            </button>
          ))}
        </div>
        {shown.length === 0
          ...
          : (
            <div className="lb-tbl">
              <div className="lb-head"><div>#</div><div>Student</div><div>Branch</div><div>Score</div><div>%</div><div>C/W/S</div></div>
              {shown.slice(0, 60).map((s, i) => {
                ...
                return (
                  <div key={i} className={`lb-row${i < 3 ? " lb-top" : ""}`}>
                    ...
                    <div className="lb-pc" style={{ color: pct >= 60 ? "#4ade80" : pct >= 40 ? "#fbbf24" : "#f87171" }}>{pct}%</div>
...
// ═══════════════════════════════════════════════
function Admin({
  setScreen, questions, setQuestions, config, setConfig,
}: {
  ...
  const onSaveQ = (q: Question) => {
    const updated = questions.find(x => x.id === q.id)
      ...
      : [...questions, q];
    doSave(updated); setTab("list"); setEditQ(null);
  };
  ...
  const doImport = () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (p.length !== 7) { setImpErr(`Line ${i + 1}: Need 7 fields separated by |`); return; }
      const [sec, text, a, b, c, d, ansStr] = p.map(x => x.trim());
      if (!SECTIONS.includes(sec as Question["section"])) {
        setImpErr(`Line ${i + 1}: Section must be Science, Math, or English`); return;
      }
      const ai = parseInt(ansStr);
      if (isNaN(ai) || ai < 0 || ai > 3) { setImpErr(`Line ${i + 1}: Answer must be 0, 1, 2, or 3`); return; }
      nqs.push({ id: uid(), section: sec as Question["section"], text, options: [a, b, c, d], answer: ai });
    }
      ...
      <div className="adm-wrap">
        <div className="adm-hdr">
          <div className="adm-hl">
            <button className="btn-bk" onClick={() => setScreen("home")}>← Exit Admin</button>
        ...
        <div className="adm-note">
          Test picks: <b>{config.science} Science</b> + <b>{config.math} Math</b> + <b>{config.english} English</b> = <b>{config.science + config.math + config.english} questions</b> per attempt. Change in ⚙ Settings.
        </div>
        ...
        <div className="tabs">
          ...
          <button className="tab tab-d" onClick={() => {
            if (window.confirm("Reset to default 100 questions? Cannot be undone.")) doSave(DEFAULT_QUESTIONS);
          }}>↺ Reset</button>
        </div>
        ...
        {tab === "list" && (
          <div className="adm-box">
            <div className="adm-tb">
              <input className="fi" placeholder="🔍 Search questions..." value={search}
                onChange={e => setSearch(e.target.value)} style={{ flex: 1 }} />
              {(["All", "Science", "Math", "English"] as SectionFilter[]).map(s => (
                <button key={s} className={`nfb${fSec === s ? " nfa" : ""}`} onClick={() => setFSec(s)}>{s}</button>
              ))}
            </div>
            <div className="ql-note">Showing {filtered.length} of {questions.length} questions</div>
            <div className="ql">
              {filtered.map(q => (
                <div key={String(q.id)} className="ql-item">
                  <div className="ql-top">
                    <span className={`sec-tag sec-${q.section}`}>{q.section}</span>
                    <div style={{ flex: 1, fontSize: ".88rem", color: "#e2e8f0", margin: "0 .6rem" }}>{q.text}</div>
                    <button className="btn-ed" onClick={() => { setEditQ(q); setTab("add"); }}>✏</button>
                    <button className="btn-dl" onClick={() => del(q.id)}>🗑</button>
                  </div>
                  <div className="ql-opts">
                    {q.options.map((o, j) => (
                      <span key={j} className={`ql-o${j === q.answer ? " ql-ok" : ""}`}>
                        {["A", "B", "C", "D"][j]}. {o}
                      </span>
                    ))}
                  </div>
              ...
              {filtered.length === 0 && <div className="empty">No questions match your filter.</div>}
            </div>
          </div>
        )}

        {tab === "add" && (
          <QForm key={editQ ? String(editQ.id) : "new"} initial={editQ}
            onSave={onSaveQ} onCancel={() => { setTab("list"); setEditQ(null); }} />
        )}

        {tab === "import" && (
          <div className="adm-box">
            <div className="imp-guide">
              <h3 style={{ color: "#f1f5f9", marginBottom: ".5rem", fontSize: "1rem" }}>📥 Bulk Import Format</h3>
              <p style={{ fontSize: ".82rem", color: "#94a3b8", marginBottom: ".75rem" }}>
                One question per line, 7 fields separated by <code>|</code>
              </p>
              <div className="imp-fmt">
                <code>Section | Question | Option A | Option B | Option C | Option D | Answer(0/1/2/3)</code>
              </div>
              <div className="imp-ex">
                <div style={{ fontSize: ".68rem", color: "#64748b", textTransform: "uppercase", fontWeight: 700, marginBottom: ".3rem" }}>Example:</div>
                <code>Science|What is the SI unit of force?|Newton|Joule|Watt|Pascal|0</code><br />
                <code>Math|What is d/dx(x²)?|x|2x|x²|2x²|1</code><br />
                <code>English|Select correct spelling:|Recieve|Receive|Recevie|Reciave|1</code>
              </div>
              <p style={{ fontSize: ".73rem", color: "#64748b", marginTop: ".5rem" }}>
                <b>Sections:</b> Science, Math, English &nbsp;|&nbsp; <b>Answer index:</b> 0, 1, 2, or 3
              </p>
            </div>
            <textarea className="imp-ta" rows={12}
              placeholder={"Science|Question here|A|B|C|D|0\nMath|Question here|A|B|C|D|2\n..."}
              value={importTxt}
              onChange={e => { setImportTxt(e.target.value); setImpErr(""); setImpOk(""); }} />
            {impErr && <div className="imp-err">⚠ {impErr}</div>}
            {impOk  && <div className="imp-ok">{impOk}</div>}
            <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
              <button className="btn-prim" style={{ flex: 1 }} onClick={doImport} disabled={!importTxt.trim()}>
                Import Questions
              </button>
              <button className="btn-out" onClick={() => setImportTxt("")}>Clear</button>
            </div>
          </div>
        )}
        ...
        {tab === "settings" && (
          <div className="adm-box">
            <h3 style={{ color: "#f1f5f9", marginBottom: ".4rem", fontSize: "1.05rem" }}>⚙ Test Configuration</h3>
            <p style={{ fontSize: ".78rem", color: "#64748b", marginBottom: "1.4rem" }}>
              Set how many questions from each section appear in each test attempt.
            </p>
            {(["science", "math", "english"] as const).map(sec => {
              const label = sec.charAt(0).toUpperCase() + sec.slice(1);
              const available = questions.filter(q => q.section === label as Question["section"]).length;
              const colorClass = sec === "science" ? "blue" : sec === "math" ? "purple" : "green";
              return (
                <div key={sec} className="cfg-row">
                  <div className="cfg-left">
                    <span className={`asp ${colorClass}`}>{label}</span>
                    <span className="cfg-avail">{available} available in bank</span>
                  </div>
                  <div className="cfg-right">
                    <button className="cfg-btn" onClick={() => setCfgEdit(c => ({ ...c, [sec]: Math.max(0, c[sec] - 5) }))}>−5</button>
                    <button className="cfg-btn" onClick={() => setCfgEdit(c => ({ ...c, [sec]: Math.max(0, c[sec] - 1) }))}>−1</button>
                    <div className="cfg-val">{cfgEdit[sec]}</div>
                    <button className="cfg-btn" onClick={() => setCfgEdit(c => ({ ...c, [sec]: Math.min(available, c[sec] + 1) }))}>+1</button>
                    <button className="cfg-btn" onClick={() => setCfgEdit(c => ({ ...c, [sec]: Math.min(available, c[sec] + 5) }))}>+5</button>
                  </div>
                </div>
              );
            })}
            <div className="cfg-total">
              Total questions per test: <b style={{ color: "#60a5fa" }}>{cfgEdit.science + cfgEdit.math + cfgEdit.english}</b>
              &nbsp;· Total marks: <b style={{ color: "#4ade80" }}>{(cfgEdit.science + cfgEdit.math + cfgEdit.english) * 2}</b>
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <button className="btn-prim" style={{ flex: 1 }} onClick={() => {
                setConfig(cfgEdit); saveConfig(cfgEdit);
                setMsg("Config saved ✓"); setTimeout(() => setMsg(""), 2500);
              }}>Save Configuration</button>
              <button className="btn-out" onClick={() => setCfgEdit({ ...DEFAULT_CONFIG })}>Reset to Default (50/30/20)</button>
            </div>
...
function QForm({
  ...
  const [section, setSection] = useState<Question["section"]>(initial?.section ?? "Science");
  const [text,    setText]    = useState(initial?.text ?? "");
  const [opts,    setOpts]    = useState<[string, string, string, string]>(
    initial ? [...initial.options] as [string, string, string, string] : [...blankOptions]
  );
  ...
  const submit = () => {
    ...
    opts.forEach((o, i) => { if (!o.trim()) e[`o${i}`] = "Required"; });
  ...
  return (
    <div className="adm-box">
      <h3 style={{ color: "#f1f5f9", marginBottom: "1.2rem", fontSize: "1.05rem" }}>
        {initial ? "✏ Edit Question" : "➕ Add New Question"}
      </h3>

      <div className="fg">
        <label className="fl">Section *</label>
        <select className="fi" value={section} onChange={e => setSection(e.target.value as Question["section"])}>
      ...
      <div className="fg">
        ...
        <textarea className={`fi fi-ta${errors.text ? " fi-e" : ""}`} rows={3}
      ...
      <div className="fg">
        <label className="fl">Options * — click letter to mark correct answer</label>
        {opts.map((opt, i) => (
          <div key={i} className={`of-row${answer === i ? " of-ok" : ""}`}>
            <button className={`of-rb${answer === i ? " of-rb-on" : ""}`} onClick={() => setAnswer(i)}>
              {answer === i ? "✓" : ["A", "B", "C", "D"][i]}
            </button>
            <input className={`fi${errors[`o${i}`] ? " fi-e" : ""}`}
